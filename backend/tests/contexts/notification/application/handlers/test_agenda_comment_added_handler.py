"""Tests for AgendaCommentAddedHandler."""
...
def _build_handler(
    *,
    ...
    handler = AgendaCommentAddedHandler(
...
class TestAgendaCommentAddedHandler:
    """Tests for AgendaCommentAddedHandler."""

    async def test_notifies_counterpart_when_organizer_comments(self) -> None:
        ...
        assert sn.sent[0][1].notification_type == NotificationType.AGENDA_COMMENT_ADDED

    async def test_notifies_organizer_when_counterpart_comments(self) -> None:
        organizer = UserId.generate()
        counterpart = UserId.generate()
        schedule = _make_schedule(organizer, counterpart)
        schedule.collect_events()
