"""Use case: Cancel a single Schedule."""

from __future__ import annotations

from dataclasses import dataclass
from datetime import UTC, datetime

from contexts.preparation.domain.schedule_repository import ScheduleRepository
from contexts.preparation.domain.value_objects import ScheduleId
from foundation.application.unit_of_work import UnitOfWork
from foundation.domain.event_dispatcher import EventDispatcher
from shared.domain.events import DomainEvent
from shared.domain.value_objects import UserId


class ScheduleNotFoundError(Exception):
    """Raised when the specified Schedule does not exist."""

    def __init__(self, schedule_id: ScheduleId) -> None:
        self.schedule_id = schedule_id
        super().__init__(f"Schedule not found: {schedule_id.value}")


@dataclass(frozen=True)
class CancelScheduleInput:
    """Input DTO for CancelScheduleUseCase."""

    schedule_id: ScheduleId
    actor_id: UserId


class CancelScheduleUseCase:
    """Application service that cancels a single Schedule.

    Both organizer and counterpart can cancel a schedule.
    """

    def __init__(
        self,
        uow: UnitOfWork,
        schedule_repo: ScheduleRepository,
        event_dispatcher: EventDispatcher,
    ) -> None:
        self._uow = uow
        self._schedule_repo = schedule_repo
        self._event_dispatcher = event_dispatcher

    async def execute(
        self,
        input_dto: CancelScheduleInput,
    ) -> None:
        """Cancel the schedule.

        Args:
            input_dto: Input parameters for cancelling a schedule.

        Raises:
            ScheduleNotFoundError: If the schedule does not exist.
            ScheduleAlreadyCancelledError: If the schedule is already cancelled.
            UnauthorizedScheduleOperationError: If actor is not a participant.
        """
        now = datetime.now(UTC)

        schedule = await self._schedule_repo.get_by_id(input_dto.schedule_id)
        if schedule is None:
            raise ScheduleNotFoundError(input_dto.schedule_id)

        schedule.cancel(actor_id=input_dto.actor_id, now=now)

        events: list[DomainEvent] = list(schedule.collect_events())

        async with self._uow:
            await self._schedule_repo.save(schedule)
            await self._uow.commit()

        await self._event_dispatcher.dispatch(events)
