...
- shadcn/ui のコンポーネントファイルでは、非コンポーネント export（`buttonVariants` 等）がコンポーネントと同居するパターンが頻出する。`react-refresh/only-export-components` 警告が出るため、export 行に `// eslint-disable-next-line react-refresh/only-export-components` を付与して抑制する。
- **レビュー時**: `pnpm lint` で `react-refresh/only-export-components` 警告が出ていないか確認する。出ている場合は上記の抑制コメントの追加を指摘する。
